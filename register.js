...
query = client.query ( 'SELECT count(*) as count ,  hash FROM userloginHash WHERE username = $1 ',[user.username ], function(err) {
...
query.on('row', function(result){
	if(!result ){ res.statusCode = 404; console.log("invalid username or password"); res.redirect('/'); }
	   ...
	   if(result.count == 0 ) {res.statusCode = 404 ; res.redirect('/');}
